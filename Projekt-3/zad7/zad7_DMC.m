...
if liczba_regulatorow==2
    ...
    lambda(1:liczba_regulatorow) = [1 400];
    bok = szerokosc/(liczba_regulatorow+1);
    mf = [fismf("trapmf",[-100 -1 -0.4 -0.2]);
      fismf("trapmf",[-0.4 -0.2 1 100]);
      ];
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    u(1:kk)=-0.66;
    y(1:kk)=-0.283777;
...
if liczba_regulatorow==3
    ...
    lambda(1:liczba_regulatorow) = [100 10 1];
    bok = szerokosc/(liczba_regulatorow+1);
    mf = [fismf("trapmf",[-100 -1 -0.6 0]);
      fismf("trapmf",[-0.6 -0.2 0 0.2]);
      fismf("trapmf",[-0.2 0.2 1 100]);
];
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    u(1:kk)=-0.75;
    y(1:kk)=-0.291798;
...
if liczba_regulatorow==4
    ...
    lambda(1:liczba_regulatorow) = [1 10 100 1];
    bok = szerokosc/(liczba_regulatorow+1);
...
if liczba_regulatorow==5
    ...
    lambda(1:liczba_regulatorow) = [1 1 100 1 100];
    bok = szerokosc/(liczba_regulatorow+1);
    mf = [fismf("trapmf",[-100 -1 -0.8 -0.333]);
      fismf("trapmf",[-0.8 -1+bok*2 -1+bok*2 0]);
      fismf("trapmf",[-0.333 -1+bok*3 -1+bok*3 0.2]);
      fismf("trapmf",[0 0.2 -1+bok*4 0.5]);
      fismf("trapmf",[0.333 0.5 1 100]);
      ];
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    u(1:kk)=-0.83;
    y(1:kk)=-0.299143;
...
for k=7:kk
    ...
    if liczba_regulatorow==3
        ...
        dU = (du1(1)*mi(1)+du2(1)*mi(2)+du3(1)*mi(3))/(mi(1)+mi(2)+mi(3));
    end
    if liczba_regulatorow==4
        du1=K1*(yzad(k)*ones(N(1),1)-y(k)*ones(N(1),1)-Mp1*dUp);
        du2=K2*(yzad(k)*ones(N(2),1)-y(k)*ones(N(2),1)-Mp2*dUp);
        ...
        dU = (du1(1)*mi(1)+du2(1)*mi(2)+du3(1)*mi(3)+du4(1)*mi(4))/(mi(1)+mi(2)+mi(3)+mi(4));
    end
    if liczba_regulatorow==5
        du1=K1*(yzad(k)*ones(N(1),1)-y(k)*ones(N(1),1)-Mp1*dUp);
        du2=K2*(yzad(k)*ones(N(2),1)-y(k)*ones(N(2),1)-Mp2*dUp);
        ...
        dU = (du1(1)*mi(1)+du2(1)*mi(2)+du3(1)*mi(3)+du4(1)*mi(4)+du5(1)*mi(5))/(mi(1)+mi(2)+mi(3)+mi(4)+mi(5));
    end
...
xlabel('k'); 
ylabel('u');
print('pust_dmc_u_reg_2_po_lambda.png','-dpng','-r400');
...
legend('y','yzad');
title("y, E="+ num2str(E))
print('pust_dmc_y_reg_2_po_lambda.png','-dpng','-r400');